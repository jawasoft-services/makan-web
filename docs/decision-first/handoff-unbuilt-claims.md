...
| City pages (`/standings/<city>`) and restaurant pages (`/places/<slug>`) | Standings, sitemap, share cards | Live 2026-09-02, same reader. A city page exists once two restaurants in it have five or more verified matchups (Durham, London, Valletta today); a restaurant page exists for every place with at least one verified matchup (88 today), with its global rank, city rank, Eats, public-meal count, up to three recent clean public captions, its own share card, Restaurant + BreadcrumbList JSON-LD. Slugs are the name plus six characters of a hash of the place id, stable. Pages are prerendered at build and revalidate hourly; new slugs render on demand. | Code |
| City and country on `/standings` | Standings page | Meals and restaurants carry no address (only 5 of 88 ranked-eligible places have coordinates), so `lib/place-city.ts` resolves Google Places address components. Seeded for all 88 eligible places in `lib/data/place-cities.json` (88 calls, Location Only SKU, 2026-09-02). Unknown ids are looked up at runtime with the site's Firebase service account as an OAuth caller, billed to the project via `X-Goog-User-Project`, cached 30 days; on Vercel this needs the service account (`makan-website@munchies-expo.iam.gserviceaccount.com`, today `roles/datastore.viewer` only) to also hold `roles/serviceusage.serviceUsageConsumer` on `munchies-expo`; the Places API (New) is enabled there. The grant command is in the launch checklist; it could not be applied from this session. Failure shows "—", never an error. Indonesian districts are normalised at read time (Jakarta; "<district>, Bali"; Depok/Bogor). | Code; Devon to confirm the SA role if a "—" appears for a new place |
