...
const credited = new Map(
  [...credits.matchAll(/^\| ([\w.-]+\.(?:jpg|webp|mp4)) \|[^|]*\|[^|]*\| (yes|no) \|/gm)].map((m) => [m[1], m[2] === "yes"]),
)
const files = []
...
for (const f of files) for (const m of readFileSync(f, "utf8").matchAll(/\/(?:meals|app-screens)\/story\/([\w.-]+\.(?:jpg|webp|mp4))/g)) referenced.add(m[1])
let failed = false
