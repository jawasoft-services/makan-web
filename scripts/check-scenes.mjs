...
const probeSource = `
  window.__probe = {
    ...
    pageLeaves(root){return this.leaves(root).filter(x=>!x.e.closest('.scene-armed')&&!x.e.closest('[role="group"]')&&!x.e.closest('.meal-carousel'))},
    collide(root,L){const LL=L||this.leaves(root);const out=[];for(let i=0;i<LL.length;i++)for(let j=i+1;j<LL.length;j++){const a=LL[i],b=LL[j];if(a.e.contains(b.e)||b.e.contains(a.e))continue;const ox=Math.min(a.r.right,b.r.right)-Math.max(a.r.left,b.r.left),oy=Math.min(a.r.bottom,b.r.bottom)-Math.max(a.r.top,b.r.top);if(ox>2&&oy>2&&a.t!=='or'&&b.t!=='or')out.push(a.t+' × '+b.t)}return out},
    imgOver(root,L){const LL=(L||this.leaves(root)).filter(x=>!x.e.closest('figure'));let imgEls=[...root.querySelectorAll('img')];if(L)imgEls=imgEls.filter(i=>!i.closest('.scene-armed')&&!i.closest('[role="group"]')&&!i.closest('.meal-carousel'));const imgs=imgEls.filter(i=>this.eff(i)>0.5).map(i=>({el:i,r:i.getBoundingClientRect()}));const out=[];for(const t of LL)for(const im of imgs){if(t.e.contains(im.el)||im.el.contains(t.e))continue;const r=im.r;const ox=Math.min(t.r.right,r.right)-Math.max(t.r.left,r.left),oy=Math.min(t.r.bottom,r.bottom)-Math.max(t.r.top,r.top);if(ox>2&&oy>2&&t.t!=='or')out.push('image over '+t.t)}return out},
...
// ---- desktop: every stage of every pinned scene
{
  ...
  const result = await page.evaluate(async () => {
    ...
    // rest-of-page pass, scenes at rest: pinned-scene stages, hero card fans and
    // the live carousel are stacked on purpose and excluded (pageLeaves)
    const main = document.querySelector("main")
    const L = P.pageLeaves(main)
    const mainHits = [...P.collide(main, L), ...P.imgOver(main, L)]
    return { out, mainHits }
  })
...
// ---- mobile: nothing overflows sideways, nothing collides with every beat on
{
  ...
  const r = await page.evaluate(() => {
    ...
    const P = window.__probe
    const main = document.querySelector("main")
    const sceneCol = [...document.querySelectorAll(".scene-armed")].flatMap((s) => [...P.collide(s), ...P.imgOver(s)])
    const L = P.pageLeaves(main)
    const restCol = [...P.collide(main, L), ...P.imgOver(main, L)]
    return { overflow: document.documentElement.scrollWidth > innerWidth, col: [...sceneCol, ...restCol] }
  })
...
console.log("check-scenes passed — no text collisions or images over text (scenes per stage, rest of the page at rest), nothing past the fold in the scenes, flights land, no mobile overflow.")
