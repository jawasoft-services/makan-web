...
const probeSource = `
  window.__probe = {
    ...
    imgOver(root,L){const LL=(L||this.leaves(root)).filter(x=>!x.e.closest('figure'));let imgs=[...root.querySelectorAll('img')];if(L)imgs=imgs.filter(i=>!i.closest('.scene-armed')&&!i.closest('[role="group"]')&&!i.closest('.meal-carousel'));imgs=imgs.filter(i=>this.eff(i)>0.5).map(i=>i.getBoundingClientRect());const out=[];for(const t of LL)for(const r of imgs){const ox=Math.min(t.r.right,r.right)-Math.max(t.r.left,r.left),oy=Math.min(t.r.bottom,r.bottom)-Math.max(t.r.top,r.top);if(ox>2&&oy>2&&t.t!=='or')out.push('image over '+t.t)}return out},
